...
export default function Myfavorites({ toast }) {
  ...
  const favoriteProfiles = Array.isArray(userprofile)
    ? userprofile.filter((profile) => profile.favoriteProfile === true)
    : [];
      ...
      {loading ? (
        ...
        <>
          <div className="w-[90%] justify-start items-center flex-col">
            <p className="text-[15px] text-[#062A27] Satoshi-bold mt-3">
              MY FAVORITES
            </p>
          </div>
            ...
            <div className="flex justify-center items-center rounded-[10px] border border-[#f0f0f0] shadow-md mt-5 flex-col w-[90%]">
              {favoriteProfiles?.map((item, index) => (
                <div key={index} className="flex w-[90%] flex-col">
                  <div
                    key={index}
                    className="flex w-[100%]  rounded-[50%] mt-5"
                  >
                    <img
        ...
        >
          <div className="flex  items-center w-[100%] flex-col">
            ...
            <div className=" flex justify-center  items-end w-[80%] mt-5">
              <p className="text-center text-[15px] text-[#062A27] Satoshi-bold">
                Are you sure you want to remove this medallion from your
                favorites?
              </p>
            </div>
            <div className="w-[85%] flex items-center justify-between  mt-5">
              <button
                onClick={handleclose}
                className="border border-[#062A27] rounded-[30px] w-[140px] h-[40px] flex justify-center items-center Satoshi-bold"
              >
                No
              </button>
              <button
                onClick={updateFavoriteStatus}
                className="bg-[#062A27] text-[white] rounded-[30px] w-[140px] h-[40px] flex justify-center items-center Satoshi-bold"
              >
                Yes
              </button>
            </div>
