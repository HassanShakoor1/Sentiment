...
export default function Medallions({toast}) {
  ...
  <FadeLoader color="#062A27" />
   ...
   {userprofile.length===0 && <div className='flex justify-center border border-[#DFE1E1] rounded-[20px] mt-5 items-center flex-col h-[250px] w-[90%]'>
    <BiSolidUser className='w-[50px] h-[50px] text-[#5F6161]' />
    <h1 className='font-bold text-[16px] mt-3'>No profile found</h1>
    <p className='text-[#5F6161] text-[13px] w-[70%] text-center mt-3'>No profile has been added yet in collection</p>
    </div>}
   {userprofile.length > 0 && userprofile.map((item, index) => (
    <div key={index} className='flex justify-center items-center rounded-[10px] border border-[f0f0f0] shadow-md mt-5 flex-col w-[90%]'>
      <div className='flex  w-[90%] mt-5 '>
        <img className='w-[80px] h-[80px] rounded-full ' src={item?.userProfile?item?.userProfile:profile1}  />
        <div className='flex flex-col ml-5 w-[80%]'>
          <p className='font-bold text-[16px] Satoshi-bold'>{item.firstName} {item.lastName} </p>
          <div className='flex items-center '><p>Relationship:</p><p className='text-[#5F6161] text-[12px] ml-1 mt-1'>{item?.relationship}</p></div>
          <div className='flex items-center'><p>Created in:</p><p className='text-[#5F6161] text-[12px] ml-1 mt-1'>{item?.createdDate}</p></div>
      ...
      <div className='w-[90%] flex items-center justify-between mb-5 mt-5'>
        <button onClick={()=> handleNavigateview (item.id)} className='border border-[#062A27] rounded-[30px] w-[47%] h-[40px] flex justify-center items-center Satoshi-bold'><img className='w-[20px] h-[20px] mr-2' src={eye} alt="view" />View</button>
        <button onClick={() => handleNavigate(item.id)} className='bg-[#062A27] text-[white] rounded-[30px] w-[47%] h-[40px] flex justify-center items-center Satoshi-bold'><img className='w-[16px] mr-2' src={edit} alt="edit" />Edit</button>
      </div>
    </div>
  ))}
   </div>
