    ...
    if (password !== confirmPassword) {
      ...
      if (email && password) {
        ...
        if (!passwordRegEx.test(password)) {
          toast.error("Password must be 8+ chars, include a special char and a capital letter");
            return; 
        }
          try {
        ...
        <>
            <div className='flex items-center justify-center flex-col w-[100%] min-h-[100vh] bg-[#f0f0f0]'>
                ...
                <div className='w-[90%] rounded-[20px] bg-white flex justify-center mt-10 items-center flex-col'>
                    ...
                    <input type='text' placeholder='First Name' value={firstName} onChange={(e) => setFirstName(e.target.value)} className='w-[90%] outline-none border border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] pl-3'/>
                    <input type='text' placeholder='Last Name' value={lastName} onChange={(e) => setLastName(e.target.value)} className='w-[90%] outline-none border border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] pl-3'/>
                    <input type='email' placeholder='Email Address' value={email} onChange={(e) => setEmail(e.target.value)} className='w-[90%] outline-none border border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] pl-3'/>
                    <div className='w-[90%] outline-none border flex items-center border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] '>
                        <input className='outline-none border-none w-[90%] p-3 h-[40px]' type={Password ? 'password' : 'text'} name='password' placeholder="Password" value={password} onChange={(e) => setPasswordValue(e.target.value)} />
                        {Password ? (
                    ...
                    <div className='w-[90%] outline-none border flex items-center border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] '>
                        <input className='outline-none border-none w-[90%] p-3 h-[40px]' type={showPassword ? 'password' : 'text'} name='password' placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
                        {showPassword ? (
                    ...
                    <button className='bg-[#062A27] rounded-[30px] h-[45px] mt-5 w-[90%] font-[600] text-[16px] cursor-pointer text-white Satoshi-bold' onClick={handleSignup}>Register</button>
