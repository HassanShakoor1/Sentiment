...
let dispatch =useDispatch();
 ...
 let [cropModal, setcropModal] = useState(false);
  ...
  />
    ...
    {userdata?.profileImage  &&<div onClick={handleopenremove} className='  rounded-full w-[20px]  h-[20px] absolute top-[5px] right-[14px] flex justify-center items-center text-sm font-[1500] text-white bg-red-600' ><RiSubtractLine/></div>}
    <label htmlFor="img" className='w-[0px] h-[0px] absolute  top-1 left-[86px]'>
    {!userdata?.profileImage &&<div className=' border rounded-full w-[22px] h-[22px] flex justify-center  text-sm font-[1500] text-white bg-[#062A27]' >+</div>}
   
    <input key={key} type="file" name="img" id='img' className='opacity-0 w-[0px] h-[0px]' onChange={handleImageChange} />

</label>
<img src={userdata?.profileImage?userdata?.profileImage :profile1}  className='rounded-full border-[3px] border-[white] w-[120px] h-[120px]' />
    </div>
  
    <div className='flex justify-start  items-start flex-col w-[59%] ml-5'>
    <h1 className='font-[600] text-[20px] text-black  mt-1 Satoshi-bold ' >{userdata?.firstName} {userdata?.lastName}</h1>
