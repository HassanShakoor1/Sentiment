...
export default function Login() {
  ...
  return (
   ...
   <input type='email' placeholder='Email Address' className='w-[90%] outline-none border border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] pl-3' value={email} onChange={(e) => setEmail(e.target.value)}/>
   <div className='w-[90%] outline-none border flex items-center border-[#C9C9C9] mt-5 h-[45px] rounded-[5px] '>
   <input className='outline-none border-none w-[90%] p-3 h-[40px]' type={showPassword ? 'password' : 'text'} name='password' placeholder="Password"  value={Password} onChange={(e) => setPassword(e.target.value)} />
   {showPassword ? (
 ...
 <Link to="/forgot" className='text-[#142C73] font-[600] text-[16px] Satoshi-bold '>Forgot password?</Link>
   ...
   <button onClick={handleLogin} className='bg-[#062A27] rounded-[30px] h-[45px] mt-3 w-[90%] font-[600] text-[16px] cursor-pointer text-white Satoshi-bold'>Log In</button>
   <p onClick={handleSignUpGoogle} className='w-[90%] cursor-pointer flex bg-[#062A27] Satoshi-bold text-white justify-between items-center outline-none border border-[#C9C9C9] mt-5 h-[45px] rounded-[30px] pl-1'>
